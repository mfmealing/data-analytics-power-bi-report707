...
Created an Orders table from Azure SQL database, cleaning the data using methods within Power BI to protect security by removing columns and improve clarity by splitting 
columns and removing any columns with a blank order date. Column names were then changed to align with Power BI naming conventions.    
...
Created a Stores table from Azure blob storage, also changing the column names to fit Power BI naming conventions.     
...
Created a table showing the 20 customers with the highest revenue, sorted by revenue with their full names, revenue and number of orders. Also added data bars to the revenue
...
added and a forecast for the next 10 periods with a confidence of 95%.    
...
Created a line chart for the total revenue using the same date hierarchy as before, including a trend line and a forecast for the next 10 periods with a confidence of 95%, as 
...
orders), the trend axis as the starts of the quarter and the target as the newly calculated targets. This was then formatted so that high was good and the colour green and 
...
set as the x-axis - this was then put against the sum of the product quantity with the description as the values and the category as the legend.    
...
Added a map visual to show each of the stores locations, using the geography hierarchy previously created and year to date profit as the bubble size.    
Created a tile style slicer at the top of the page to filter by country, also including a select all option.    
Built a drillthrough page which could be viewed when right clicking clicking on a particular bubble on the map. This included a table with the top 5 products by year to date profit, 
including the columns of description, year to date profit, total orders and total revenue, sorted alphabetically. There was also a column chart for the total orders by category,
a card visual showing the selected store and two gauge visuals, one for the year to date profit and one for the year to date revenue, both showing targets of a 20% increase 
...
bar chart didn't affect the customers line graph and the total customers by country donut chart cross-filtered the bar chart. The Product Detail page was changed so that the 
...
hovering over the buttons, they would change colour by swapping out for the same images but in a different colour. The action of each of the buttons was then changed to direct to 
